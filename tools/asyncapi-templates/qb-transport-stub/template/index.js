...
/*
 * Extracts the machine-readable transport facts from the AsyncAPI document
 * and emits them as a Python module and a Java class. Throws (failing the
 * whole generation) when the spec loses a structure the protocol relies on.
 */
function extractFacts(asyncapi) {
  ...
  for (const channel of asyncapi.channels().all()) {
    ...
    if (amqp.is === 'routingKey' && amqp.exchange) {
      ...
      else if (address.startsWith('responses.')) responsesQueue = queue;
      else if (address === 'pds.dlq') dlq = queue;
    }
  }
...
export default function ({ asyncapi }) {
  ...
  return [
    <File name="qb_stub.py">{pythonStub(facts)}</File>,
    <File name="BrokerTransportSpec.java">{javaStub(facts)}</File>,
  ];
}
